...
## 五、WebView2 说明（离线安装器已随包）

`DshDesktop.exe` 的内嵌浏览器内核需要系统的 **Edge WebView2 运行时**。
Windows 11 与现代 Windows 10（带 Edge）通常已预装，**不需要做任何事**。

若目标机确实缺失（桌面壳会弹出"需要 WebView2 运行时"并尝试联网下载 —— 内网会失败），
两种办法任选：

1. **用随包的离线安装器**（推荐）：进入 `vendor\webview2\`，双击
   `install-webview2.bat`（或直接运行 `MicrosoftEdgeWebView2RuntimeInstallerX64.exe`）。
   该安装器为 Microsoft 官方 Evergreen 独立版（版本 1.3.265.7，202.9 MB），
   **装到当前用户、无需管理员、无需联网**。
2. **改用系统浏览器**：点桌面壳上的「浏览器打开」，或直接双击 `start-dsh.bat`，
   用系统浏览器访问 `http://127.0.0.1:3099` —— 功能完整，不依赖 WebView2。
